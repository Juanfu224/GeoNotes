...
public record GeoPoint(double lat, double lon) {
    // El 'constructor compacto' de un record se usa para validar los datos de los
    // campos.
    // Es una buena práctica para asegurar la consistencia del objeto.
    public GeoPoint {
        if (lat < -90 || lat > 90)
            throw new IllegalArgumentException("Latitud inválida: " + lat);
        if (lon < -180 || lon > 180)
            throw new IllegalArgumentException("Longitud inválida: " + lon);
    }
}